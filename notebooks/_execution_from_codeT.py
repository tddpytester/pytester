...
def check_correctness_with_test_cases(task_id, prompt, completion, test_cases, fn_name, timeout):
    """
    Evaluates the functional correctness of a solution_content by running the test
    suite provided in the problem. 
    """
    extend_timeout = timeout*len(test_cases)
    test_cases = split_test_cases(test_cases, fn_name)
    def unsafe_execute():
        with create_tempdir():
            ...
            check_program = (
                prompt + completion + "\n" +
                _pack_test_cases(test_cases, fn_name, timeout)
            )
            try:
